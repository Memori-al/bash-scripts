...
if [[ "$1" == "oracle" ]]; then
    ...
    echo -e $blue$bold"sorting$cls  : db data sorting"
elif [[ "$1" == "apm" ]]; then
    ...
    echo -e $blue$bold"send NICK NAME ID$cls : send luikie NAME 15"
elif [[ "$1" == "help" ]]; then
    echo -e "$usage {rpm|repo|apm|oracle|quota|sendmail|mail}"
    ...
    echo -e $blue$bold"sendmail$cls  : sendmail install & composition"
    echo -e $blue$bold"mail$cls      : score-result.txt send"
elif [[ "$1" == "quota" ]]; then
    echo -e "$usage {install|set}"
    echo -e $blue$bold"install$cls  : oracle preinstall & 21c ee install"
    echo -e $blue$bold"set$cls          : quota options setting"
else
    echo -e "$title : /sm-shell/Resource/Scripts/$(basename $0) $tear'$1'$cls is available command."
    echo  
    echo -e "$usage {rpm|repo|apm|oracle|quota|sendmail|mail}"
    echo -e $blue$bold"rpm$cls       : rpm -qa list output"
    echo -e $blue$bold"repo$cls       : local repository composition"
